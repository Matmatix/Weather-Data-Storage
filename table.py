...
class HashTable:
    def __init__(self, length, hash):
        if isinstance(length) != int or length <= 0: #make the table at least one long
            length = 1
        if isinstance(hash) != int or hash < 0 or hash > 1:           #this integer value(can possibly change later) will be used to choose a hashing alogrithm for this specific hash table
            hash = 0
        self.hash = hash
        self.length = length
