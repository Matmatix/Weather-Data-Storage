...
class HashTable:
    ...
    def add(self, key, object)
        ...
        else if type(self.slots[location]) != list:             #if the slot already has a singular object in it replace with a list 
    ...
    def update(self, key, update)  
        ...
        if self.slots[location] == None:                        #if there is nothing to replace do nothing
        else if type(self.slots[location]) != list:             #if there is only a single object there replace it
